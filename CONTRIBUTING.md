...
Incoming `scroll:sync` messages are coalesced with latest-wins `requestAnimationFrame` scheduling.
The receiver applies only the newest pending target, updates `lastSyncedRatio` at the actual apply
time, and uses `applyInstantProgrammaticScroll()` so page-level `scroll-behavior: smooth` does not
animate extension-driven sync updates. The helper temporarily sets inline `scrollBehavior: auto` on
the scroll root and `document.body` when distinct, applies `scrollTop`, then restores the previous
inline values. It must not disable smooth scrolling globally.
...
Use this checklist when touching `src/contentScripts/scroll-sync.ts`,
`src/contentScripts/lib/instant-programmatic-scroll.ts`, timing constants, or manual offset
baseline logic:

1. Build or run the extension with `pnpm dev`.
2. Open two synced tabs where the page sets `scroll-behavior: smooth`, for example two MDN language
   variants of the same document.
3. Scroll rapidly in the source tab and confirm the receiver follows the latest position immediately
   instead of animating through old targets.
4. While the receiver is catching up, keep scrolling the source tab and confirm the source remains
   responsive instead of being pulled back to a stale receiver position.
5. Hold Option/Alt to create a manual offset, release it, and confirm future sync uses the new
   alignment.
6. Click an in-page anchor and confirm the page's own smooth scrolling behavior is not globally
   disabled after extension sync updates.
...
## 수동 QA: Smooth-scroll 페이지

`src/contentScripts/scroll-sync.ts`, `src/contentScripts/lib/instant-programmatic-scroll.ts`, 타이밍
상수, manual offset baseline 로직을 수정했다면 다음을 확인하세요.

1. `pnpm dev`로 확장 프로그램을 실행하거나 빌드합니다.
2. MDN의 서로 다른 언어 문서처럼 `scroll-behavior: smooth`를 설정한 페이지 두 개를 동기화합니다.
3. 발신 탭을 빠르게 스크롤했을 때 수신 탭이 오래된 target을 애니메이션으로 순차 재생하지 않고
   최신 위치를 즉시 따라가는지 확인합니다.
4. 수신 탭이 따라가는 중에도 발신 탭을 계속 스크롤할 수 있고, stale receiver 위치로 되튕기지
   않는지 확인합니다.
5. Option/Alt로 manual offset을 만든 뒤, 이후 동기화가 새 정렬 기준을 사용하는지 확인합니다.
6. 페이지 내부 anchor를 클릭했을 때, 확장이 페이지 자체 smooth scrolling을 전역으로 꺼버리지
   않았는지 확인합니다.
