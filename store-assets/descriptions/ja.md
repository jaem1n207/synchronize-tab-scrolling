...
ポップアップを開かずに同期を開始したり、タブを追加したりできます。

1. 最初のタブで`Cmd+Shift+.`（Mac）または`Ctrl+Shift+.`（Windows/Linux）を押します。
2. 10秒以内に別のタブで同じショートカットを押すと、同期が始まります。
3. 同期中は、追加したいタブで同じショートカットを押します。

ショートカットは、ポップアップの「アクション」メニューから変更できます。

拡張機能はArcでも利用できますが、Arcは標準のWebExtension `commands.onCommand`イベントを拡張機能へ渡さないため、Quick Syncショートカットには対応していません。Arcではポップアップでタブを選び、「同期開始」を押してください。

詳細: https://github.com/jaem1n207/synchronize-tab-scrolling/blob/main/docs/guides/quick-sync-shortcut.md
...
• Quick Syncショートカットで、ポップアップを開かずに同期を開始してタブを追加
