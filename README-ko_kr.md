...
로컬 파일은 수동 동기화만 지원하며, 자동 동기화 제안은 의도적으로 `file://` 탭을 묶지 않습니다.
PDF와 Word 파일은 계속 지원하지 않아요.
...
키를 놓은 뒤에는 방금 맞춘 기준점에서의 signed pixel 차이를 유지해요. 두 페이지의 헤더, 목차,
광고, 번역 길이가 달라도 이미지, 캡션, 가까운 문단이 더 안정적으로 같은 높이에 머물 수 있습니다.
...
대부분의 일반 `http(s)` 페이지와 브라우저가 읽을 수 있는 `file://` 문서는 선택할 수 있어요.
다만 브라우저가 확장 프로그램 접근을 막거나, 페이지가 content script를 받을 수 없거나, 확장
프로그램이 노이즈/개인정보 위험이 큰 맥락을 의도적으로 피하는 경우에는 사용할 수 없습니다:

- 브라우저/내부/확장 프로그램 페이지 (`chrome://`, `edge://`, `about:`, `devtools://`,
  `chrome-extension://`, `moz-extension://`)
- 브라우저 확장 프로그램 스토어 (Chrome Web Store, Edge Add-ons, Firefox Add-ons)
- Google Workspace, 계정, 콘솔 서비스 (Docs, Drive, Gmail, Sheets, Slides, Calendar, Meet,
  Photos, Accounts, Cloud Console 등)
- 검색 결과 페이지 (Google Search, Naver Search, Bing, DuckDuckGo, Yahoo, Baidu, Daum 등)
- 호환되지 않는 것으로 알려진 앱 화면 (Figma, Notion, YouTube Music, Atlassian Jira/Confluence)
- PDF 파일과 PDF viewer 경로, 로컬 `.pdf` 파일
- 로컬 Word 문서 (`file://` `.doc` / `.docx`)
- 로그인/인증 페이지 (`login.`, `auth.`, `sso.`, `/login`, `/auth`, `/oauth` 같은 일반 패턴 포함)
- 특수 URL scheme (`view-source:`, `data:`, `blob:`, `filesystem:`, `javascript:`, `vbscript:`,
  `ftp:`, `ws:`, `wss:`)

이런 탭은 선택 목록에서 비활성화돼요. 예외적으로 브라우저에서 읽을 수 있는 로컬 파일은 file URL
접근 권한이 허용되어 있으면 수동 동기화에 사용할 수 있습니다. Chromium 계열 브라우저에서 file
access가 꺼져 있으면, 팝업이 **"파일 URL에 대한 액세스 허용"** 설정으로 이동하는 버튼을 보여줘요.
...
새 수동 정렬은 문서 끝까지 남은 길이를 다시 비율로 나누지 않고, 방금 맞춘 지점 기준의 signed
pixel delta를 유지합니다. 이전 버전에서 저장된 정렬값은 호환성을 위해 기존 proportional mapping을
그대로 사용합니다.
...
페이지 이동 방식은 세 가지 중에서 고를 수 있어요.

- **변경한 탭 따라가기**: 다른 탭도 변경한 탭이 연 웹사이트/페이지로 이동해요. 검색/결과 페이지의
  query parameter처럼 페이지를 식별하는 query도 함께 반영하며, 대상 탭의 언어 표시가 URL에 있으면
  그 언어 표시는 보존합니다.
- **각 탭의 웹사이트 유지**: 각 탭은 자기 웹사이트에 남고, 가능한 경우 같은 페이지 경로와 페이지를
  식별하는 query를 열어요. 이동하기 전에는 원본 탭과 대상 탭의 사이트 경계가 호환되는지도 확인합니다.
  원문과 다른 사람이 번역한 글처럼 서로 다른 URL 체계를 가진 사이트 조합이라면 대상 탭은 현재 페이지에
  그대로 남고, 안내 메시지를 보여준 뒤 스크롤 동기화만 계속 유지합니다.
...
자동 동기화 제안은 **기본적으로 꺼져 있어요**. **작업 메뉴**에서 **"같은 페이지 탭 자동 제안"**을
켜면, 확장 프로그램이 같은 페이지로 보이는 탭을 발견했을 때 각 탭의 오른쪽 하단에 토스트 알림을
보여줄 수 있습니다. 이 기능은 스크롤 동기화를 시작하자는 제안일 뿐이고, 동기화 시작 후 페이지
이동을 따라갈지 정하는 **"페이지 이동도 동기화"**와는 별도예요.

```mermaid
flowchart LR
    A[같은 페이지 탭 자동 제안 켜기] --> B[같은 페이지 탭 열기]
    B --> C[토스트 표시]
    C --> D{사용자 선택}
    D -->|동기화 시작| E[동기화 시작됨]
    D -->|지금 안 함| F[세션 동안 무시됨]
    D -->|이 사이트에서 표시 안 함| G[도메인 영구 제외]
    C --> H[10초 후 자동 사라짐]
```

- **작업 메뉴** → **"같은 페이지 탭 자동 제안"**에서 전체 활성화/비활성화할 수 있어요
