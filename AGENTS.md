...
10. **Runtime-validate storage values** — `browser.storage.local` values are untrusted. Use type guards or `typeof` checks instead of type assertions. For `autoSyncEnabled`, only explicit booleans are valid; missing, malformed, or unreadable values must behave as `false`.
...
- `@ts-ignore`, `as any`, or convenience type assertions like `as boolean` — validate and narrow the type
