...
- Manifest V3 (Chrome/Edge/Brave/Firefox)
...
| D3  | 크로스 브라우저      | 로컬 파일 탭 연결 실패     | Chromium 파일 URL 접근 꺼짐 | 설정 CTA + 실패 탭 기준 안내      |
...
### D3. 로컬 파일 탭 연결 실패와 파일 URL 접근 권한

**커밋:** `a0965dd`, `9e9c76d`, `49fe3df`, `0707622` |
**파일:** `src/shared/lib/file-scheme-access.ts`, `src/popup/hooks/use-tab-discovery.ts`,
`src/popup/hooks/use-sync-control.ts`

#### 검색 키워드

`file://`, `local file`, `Allow access to file URLs`, `isAllowedFileSchemeAccess`,
`chrome://extensions/?id=`, `edge://extensions/?id=`, `로컬 파일`, `파일 URL 접근`,
`Could not establish connection`

#### 배경

Chrome, Edge, Brave, Arc 같은 Chromium 브라우저는 확장 프로그램이 `file://` 페이지에 콘텐츠
스크립트를 주입하려면 사용자가 확장 프로그램 상세 페이지에서 별도 토글을 켜야 한다. Manifest에
`file:///*` match pattern이 있어도 이 토글이 꺼져 있으면 실제 연결은 실패한다.

#### 문제 상황

- 로컬 HTML 보고서, Markdown, JSON, 텍스트, CSV, 로그 파일을 Chrome에서 열 수 있음
- 팝업에서 해당 탭이 보이지만 파일 URL 접근이 꺼져 있으면 콘텐츠 스크립트 연결 실패
- 혼합 선택에서 HTTPS 탭만 실패했는데도 로컬 파일 안내가 뜨면 실제 원인을 가릴 수 있음

#### 원인 분석

로컬 파일 동기화에는 세 조건이 모두 필요하다:

1. Generated manifest에 `file:///*`가 포함되어 있어야 한다.
2. URL eligibility가 브라우저에서 읽을 수 있는 `file://` 파일을 허용해야 한다.
3. Chromium 사용자가 **Allow access to file URLs**를 켜야 한다.

Firefox나 API를 제공하지 않는 브라우저에서는 file URL access 상태를 확정적으로 알 수 없으므로,
팝업은 실제 content script 연결 결과에 맡기고 Chrome 전용 토글이 있다고 단정하지 않는다.

#### 해결 과정

- `src/manifest.ts`에서 `host_permissions`, `content_scripts.matches`,
  `web_accessible_resources.matches`에 `file:///*` 추가
- `src/shared/lib/url-utils.ts`에서 `file://` blanket block 제거
- PDF, 로컬 Word 문서(`.doc`, `.docx`), `about:`, `data:`, `blob:`, `view-source:`는 계속 차단
- `getFileSchemeAccessInfo()`로 Chromium의 `chrome.extension.isAllowedFileSchemeAccess()`를 감싼다
- 파일 접근이 꺼져 있으면 팝업 row와 sync-start error에 settings CTA 표시
- sync-start error는 `connectionResults`에서 실제 실패한 selected `file://` tab이 있을 때만 파일 접근 안내로 대체
...
Expected: `file:///*`가 `host_permissions`, `content_scripts.matches`,
`web_accessible_resources.matches`에 모두 존재한다.

Manual QA:

1. Chrome/Edge/Brave/Arc에서 unpacked extension 로드
2. 파일 URL 접근 토글을 끈 상태로 로컬 `.html`, `.md`, `.json`, `.txt`, `.csv`, `.log` 파일 열기
3. 팝업에서 로컬 파일 탭이 settings action과 함께 unavailable로 표시되는지 확인
4. settings action이 `chrome://extensions/?id=<runtime.id>` 또는
   `edge://extensions/?id=<runtime.id>`를 여는지 확인
5. **Allow access to file URLs**를 켜고 팝업을 다시 열어 로컬 파일 탭 선택 가능 여부 확인
6. 두 개 이상의 로컬 파일 탭으로 manual scroll sync 확인
7. `file://` PDF와 `.doc`/`.docx` 파일은 계속 unavailable인지 확인

#### 주의

- Auto-sync suggestion은 `file://` 페이지를 자동 그룹화하지 않는다. 로컬 파일 경로 노출과 잘못된
  자동 매칭을 피하기 위해 manual sync만 지원한다.
- 로컬 파일 내용은 읽거나 업로드하지 않는다. 동기화에는 scroll position과 tab id만 사용한다.
- settings CTA는 Chromium에서 file access 상태를 확인할 수 있을 때만 확정적으로 표시한다.
...
`다크 모드`, `상태 동기화`, `스크롤 동기화`, `탭 검증`, `크로스도메인`, `불투명도`, `텍스트 색상`, `CSS 변수`, `메시지 패싱`, `이벤트 핸들링`, `포커스 없는 탭`, `수동 스크롤`, `분할 화면`, `크로스 브라우저`, `로컬 파일`, `파일 URL 접근`, `큰따옴표`, `특수문자 이스케이프`, `브라우저별 빌드`, `manifest 자동화`

### 영어 키워드

`Shadow DOM`, `UnoCSS`, `@property`, `CSS variables`, `inherits`, `opacity`, `MutationObserver`, `tabs.onUpdated`, `hard navigation`, `cross-domain`, `throttle`, `debounce`, `scroll sync`, `final position`, `Tab ID`, `discarded`, `stale state`, `validation`, `webext-bridge`, `background script`, `message passing`, `broadcast`, `WheelEvent`, `MouseEvent`, `altKey`, `metaKey`, `modifier key`, `unfocused tab`, `manual scroll`, `file://`, `local file`, `Allow access to file URLs`, `isAllowedFileSchemeAccess`, `Arc browser`, `split view`, `chrome.tabs.query`, `browser.tabs.query`, `Promise wrapper`, `callback vs promise`, `CSS selector escape`, `querySelector SyntaxError`, `MV3 CSP`, `rollup plugin`
