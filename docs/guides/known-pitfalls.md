...
파이프라인 지연 구성:
  ...
  + 메시지 중계 (2-15ms)
  + receiver rAF coalescing (최대 1 frame, ~16ms)
  + 브라우저 지터 (0-50ms, GC/렌더링)
  = ~68-135ms

PROGRAMMATIC_SCROLL_GRACE_PERIOD = 200ms → ~65-132ms 여유
```
...
- `lastSyncedRatio`는 메시지 수신 시점이 아니라 실제 프로그램 스크롤 적용 시점에 갱신합니다.
- manual baseline을 잡기 전에는 pending receiver target을 취소합니다.
- `scrollBehavior` 우회는 scroll root와, 별도 element일 때 `document.body`에만 짧게 적용하고
  기존 inline 값을 복원합니다.
...
- [ ] 수신 측 프로그램 스크롤이 `applyInstantProgrammaticScroll()`을 통해 페이지 smooth scrolling을 우회하는가?
- [ ] 연속 `scroll:sync` 메시지가 frame당 최신 target 하나만 적용하도록 coalescing되는가?
- [ ] `lastSyncedRatio`가 메시지 수신 시점이 아니라 실제 프로그램 스크롤 적용 시점에 갱신되는가?
- [ ] manual baseline, reset, stop 전환 전에 pending receiver target이 취소되는가?
