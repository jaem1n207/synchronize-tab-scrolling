...
## Pitfall 10: 플랫폼별 렌더링 차이로 인한 저대비 하이라이트 불가시화

### 규칙

> `presetShadcn('neutral')`이 기본 생성하는 `--accent: 0 0% 96.1%`는 `--background: 0 0% 100%` 대비 밝기 차이가 3.9%에 불과합니다. 이 대비는 Mac Retina 디스플레이에서는 감지되지만, 일반적인 Windows LCD/TN 패널에서는 호버/키보드 포커스 하이라이트가 전혀 보이지 않습니다. `--accent`와 `--muted`는 표면색 대비 최소 **7% 이상**의 밝기 차이를 유지해야 합니다.

### 배경

`cmdk` `CommandItem`은 활성 항목에 `data-selected="true"`를 부여하고, shadcn 기본 스타일(`data-[selected='true']:bg-accent`)이 `--accent`를 배경으로 칠합니다. 하지만 neutral 팔레트의 light 토큰은 표면색과 거의 동일하여 Mac이 아닌 환경에서는 하이라이트가 시각적으로 사라지는 문제가 발생했습니다.

```
❌ BAD: shadcn neutral 기본값 그대로
--background: 0 0% 100%   /* #ffffff */
--accent:     0 0% 96.1%  /* #f5f5f5 — 델타 3.9% */

✅ GOOD: Windows LCD에서도 구분 가능한 대비
--background: 0 0% 100%
--accent:     0 0% 90%    /* #e5e5e5 — 델타 10% */
```

### 왜 문제인가?

1. **OS/디스플레이별 렌더링 편차**: Mac Retina는 sRGB 색 프로파일이 정교하지만, Windows 기본 LCD는 색 보정이 부족하여 3.9% 밝기 차이가 동일한 색으로 보임
2. **개발자 환경 편향**: 대부분의 시각적 테스트가 Mac에서 이루어져 문제가 드러나지 않음
3. **UX 블로커**: 호버/키보드 탐색 피드백이 사라지면 사용자가 어떤 항목에 포커스가 있는지 알 수 없음

### 적용 원칙

- `--accent`, `--muted` 값을 변경할 때는 `--background`(또는 사용되는 표면색) 대비 밝기 차이가 7% 이상인지 확인
- 새로운 theme block을 추가할 때도 동일한 기준 적용
- 정적 회귀 테스트(`src/shared/styles/__tests__/theme-contrast.test.ts`)가 모든 theme block을 검증하므로 값을 완화하면 테스트가 실패

### 토큰 위치

| 컨텍스트         | 파일                                      | 라이트 블록      | 다크 블록       |
| ---------------- | ----------------------------------------- | ---------------- | --------------- |
| Popup UI         | `src/shared/styles/main.css`              | `:root { ... }`  | `.dark { ... }` |
| Content panel    | `src/contentScripts/panel.tsx`            | `.light { ... }` | `.dark { ... }` |
| Suggestion toast | `src/contentScripts/suggestion-toast.tsx` | `.light { ... }` | `.dark { ... }` |
...
- [ ] `--accent`/`--muted` 변경 시 표면색 대비 밝기 차이가 7% 이상인가? (`theme-contrast.test.ts` 통과)
